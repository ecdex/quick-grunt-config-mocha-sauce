...
helpers = {
  ...
  queuePageJsForLoad: function (filenameArrayString) {
    return new hbs.SafeString(
        "<script>pageJs = " + filenameArrayString + ";</script>"
    );
  },

  loadQueuedPageJs: function () {
    if (inProductionLikeEnvironment()) {
      return new hbs.SafeString(
          "<script>" +
          "  if (typeof pageJs !== 'undefined') {" +
          "    pageJs.forEach(function (filename) {" +
          "      var script = document.createElement('script');" +
          "      script.id = filename + '-load';" +
          "      script.src = '/a/js/' + filename + '.js';" +
          "      script.async = false;" +
          "      document.head.appendChild(script);" +
          "    });" +
          "  }" +
      ...
      return new hbs.SafeString(
          ...
          "  if (typeof pageJs !== 'undefined') {" +
          "    pageJs.forEach(function (filename) {" +
          "      loadingJs(filename, '/a/js/' + filename + '.js');" +
          "    });" +
          "  }" +
          "</script>"
      );
