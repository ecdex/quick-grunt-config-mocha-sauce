...
helpers = {
  installResourceLoadingErrorCheckFunctions: function () {
    ...
    return new hbs.SafeString([
      ...
      "      resourcesWeShouldHaveLoaded[nickname] = 'loaded';",
      "      if (typeof scriptLoadHandler === 'function') {",
      "        scriptLoadHandler();",
      "      }",
      "    };",
      "    script.src = url;",
  ...
  loadCss: function (nickname, options) {
    if (inProductionLikeEnvironment()) {
      return new hbs.SafeString("<link rel=\"stylesheet\" href=\"" + helpers.assetLoadPath(options.hash.production_path) + "\">");
    } else {
      return new hbs.SafeString([
        "<link rel=\"stylesheet\" id=\"" + nickname + "-load\" href=\"" + helpers.assetLoadPath(options.hash.development_path) + "\">",
  ...
  loadJs: function (nickname, options) {
    if (inProductionLikeEnvironment()) {
      return new hbs.SafeString("<script src=\"" + helpers.assetLoadPath(options.hash.production_path) + "\"></script>");
    } else {
      return new hbs.SafeString(
