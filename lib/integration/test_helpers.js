...
var helpers,
    ...
    /*jshint -W061 */
    config = eval(
        process.env.MOCHA_SAUCE_CONFIG_REQUIRE_SNIPPET ||
...
helpers = {
  ...
  getSauce: function () {
    ...
    var tmpDirListing = fs.readdirSync("/tmp"),
        scPidFile = _.filter(tmpDirListing, function (fileName) {
    ...
    driver.getSession().then(function (session) {
      ...
      if (!process.env.TRAVIS_BRANCH) {
        var regex = /^\* (.+)$/,
            ...
            current = _.first(_.filter(branches, function (line) {
